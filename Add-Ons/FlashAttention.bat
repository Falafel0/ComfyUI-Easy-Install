...
REM cd %DIR_LVL%
REM if exist ".\Add-Ons\Tools\Helper-CEI\ComfyUI-EZi-Desktop.ico" if exist ".\ComfyUI-EZi FlashAttention.bat" (
	REM echo %green%:::::::::::::: Creating EZi FlashAttention Desktop shortcut%reset%
	REM powershell -NoProfile -ExecutionPolicy Bypass -command "$s=(New-Object -ComObject WScript.Shell).CreateShortcut('%DESKTOP%\ComfyUI-EZi FA.lnk'); $s.TargetPath='%cd%\ComfyUI-EZi FlashAttention.bat'; $s.WorkingDirectory='%cd%\'; $s.IconLocation='%cd%\Add-Ons\Tools\Helper-CEI\ComfyUI-EZi-Desktop.ico'; $s.Save();"
REM )
