        ...
        for hive in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
            try:
                with winreg.OpenKey(hive, key_path) as k:
                    exe_path = winreg.QueryValue(k, None)
...
def _save_settings(settings):
    try:
        if not settings or not isinstance(settings, dict):
            return
        ALLOWED = ("last_save_dir", "window_maximized", "comfy_storage", "hide_deprecation_warnings", "window_placement", "custom_file_browser", "theme", "console_detached", "console_placement", "_dpi_aware_version", "rec_fps", "console_bg_image", "console_bg_fit", "console_bg_opacity", "cached_comfy_stable_version", "pinned_packages")
        existing = {}
        try:
            if os.path.exists(SETTINGS_PATH):
                with open(SETTINGS_PATH, "r", encoding="utf-8") as f:
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #modal-box {
    ...
    padding: 22px 24px 18px; min-width: 320px; max-width: 600px;
...
function show_settings() {
  ...
  const _tabDisplay = {advanced:'flex', appearance:'block'};
  function _showTab(name) {
    ['general','addons','torch','tools','appearance','advanced'].forEach(t => {
  ...
  const tabBar = `<div style="margin-bottom:14px;border-bottom:1px solid var(--border);padding-bottom:8px">
    <div style="display:flex;gap:4px;margin-bottom:6px;align-items:center;flex-wrap:wrap">
      ...
      <button id="stabtn-appearance" class="stab-btn">Appearance</button>
      ...
      <button onclick="pywebview.api.open_url('https://github.com/Tavris1/ComfyUI-Easy-Install')" 
              ...
              style="padding:4px 14px;font-family:inherit;font-size:11px;font-weight:bold;border:1px solid var(--accent);border-radius:4px;background:var(--btn-bg);color:var(--accent);cursor:pointer;transition:all .15s;line-height:1.4;white-space:nowrap;margin-left:auto;"
  ...
  const tabGeneral = `<div id="stab-general">
    ...
    <div style="margin-top:14px;border:1px solid var(--accent);border-radius:6px;overflow:hidden">
      ...
      <div style="padding:8px 10px;display:flex;flex-direction:column;gap:6px">
        <div id="cache-warning" style="display:none;align-items:center;gap:6px;padding:5px 8px;border-radius:4px;background:rgba(64,156,255,0.08);border:1px solid rgba(64,156,255,0.30)">
          <span style="font-size:13px;line-height:1">💡️</span>
          <span style="font-size:10px;color:#4da3ff;line-height:1.4">This will clear pip and uv cache files to free disk space.</span>
        </div>
  ...
  const tabAppearance = `<div id="stab-appearance" style="display:none">
  ...
  const tabAdvanced = `<div id="stab-advanced" style="display:none;flex-direction:column;gap:8px">
    ...
    <div id="sec-level-section" style="border:1px solid var(--accent);border-radius:6px;overflow:hidden">
      <div style="font-size:10px;font-weight:bold;text-transform:uppercase;letter-spacing:.06em;color:var(--text2);background:var(--section-bg,var(--input-bg));padding:4px 10px;border-bottom:1px solid var(--border2)">ComfyUI-Manager Security Level</div>
      <div style="padding:6px 10px;display:flex;flex-direction:column;gap:2px">
        <div class="settings-row" style="gap:8px">
          <label for="set-security-level" style="cursor:default;flex:none;white-space:nowrap">Security level</label>
          <select id="set-security-level" style="flex-shrink:0;max-width:130px;background:var(--input-bg);color:var(--text);border:1px solid var(--border2);border-radius:4px;font-family:inherit;font-size:11px;padding:3px 6px;cursor:pointer" onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
            <option value="weak"    id="sec-opt-weak">weak</option>
            <option value="normal-" id="sec-opt-normalminus">normal-</option>
            <option value="normal"  id="sec-opt-normal">normal</option>
            <option value="strong"  id="sec-opt-strong">strong</option>
          </select>
          <span id="set-security-loading" style="font-size:10px;color:var(--text2);font-style:italic">loading…</span>
          <button id="sec-info-btn" title="Security level info"
            style="flex-shrink:0;width:20px;height:20px;border-radius:50%;border:1px solid var(--accent);background:var(--btn-bg);color:var(--accent);font-size:11px;font-weight:bold;cursor:pointer;line-height:1;padding:0;display:inline-flex;align-items:center;justify-content:center;transition:background .15s,border-color .15s,color .15s;"
            onmouseover="this.style.background='var(--accent-bg)';this.style.borderColor='var(--accent-hover)';this.style.color='var(--accent-hover)';"
            onmouseout="this.style.background='var(--btn-bg)';this.style.borderColor='var(--accent)';this.style.color='var(--accent)';">i</button>
          <div id="sec-info-tooltip" style="display:none;position:absolute;z-index:300000;background:var(--modal-bg);border:1px solid var(--border2);border-radius:6px;padding:10px 12px;font-size:11px;color:var(--text);line-height:1.6;max-width:320px;box-shadow:0 4px 16px rgba(0,0,0,0.5);white-space:normal;">
            <b style="color:var(--accent)">strong</b> - blocks high and middle risk features<br>
            <b style="color:var(--accent)">normal</b> - blocks only high risk features<br>
            <b style="color:var(--accent)">normal-</b> - blocks high risk features only when <code style="color:#f1fa8c">--listen</code> is set outside 127.x<br>
            <b style="color:var(--accent)">weak</b> - all features are available
          </div>
        </div>
      </div>
    </div>
    ...
    <div style="border:1px solid var(--accent);border-radius:6px;overflow:hidden">
      <div style="font-size:10px;font-weight:bold;text-transform:uppercase;letter-spacing:.06em;color:var(--text2);background:var(--section-bg,var(--input-bg));padding:4px 10px;border-bottom:1px solid var(--border2)">Pinned Packages Manager</div>
      <div style="padding:6px 10px;display:flex;flex-direction:column;gap:4px">
        <div id="pinned-list" style="display:flex;flex-direction:column;gap:3px"></div>
        <div style="display:flex;gap:6px;margin-top:2px">
          <input type="text" id="pinned-new-input" placeholder="e.g. numpy==1.26.4"
            style="flex:1;min-width:0;background:var(--input-bg);color:var(--text);border:1px solid var(--border2);border-radius:4px;font-family:inherit;font-size:11px;padding:3px 6px;outline:none;"
            onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'"
            onkeydown="if(event.key==='Enter'){document.getElementById('pinned-add-btn').click();}">
          <button id="pinned-add-btn"
            style="flex-shrink:0;padding:3px 12px;font-family:inherit;font-size:11px;font-weight:bold;border:1px solid var(--accent);border-radius:4px;background:var(--btn-bg);color:var(--accent);cursor:pointer;transition:all .15s;line-height:1.4;"
            onmouseover="this.style.background='var(--accent-bg)';this.style.borderColor='var(--accent-hover)';this.style.color='var(--accent-hover)';"
            onmouseout="this.style.background='var(--btn-bg)';this.style.borderColor='var(--accent)';this.style.color='var(--accent)';">Add</button>
        </div>
      </div>
    </div>
    ...
    <div style="border:1px solid var(--accent);border-radius:6px;overflow:hidden">
      <div style="font-size:10px;font-weight:bold;text-transform:uppercase;letter-spacing:.06em;color:var(--text2);background:var(--section-bg,var(--input-bg));padding:4px 10px;border-bottom:1px solid var(--border2);display:flex;align-items:center;gap:6px">
        <span>Server — Startup Arguments</span>
        <button id="startup-args-info-btn" title="Show all available startup arguments"
          style="flex-shrink:0;width:16px;height:16px;border-radius:50%;border:1px solid var(--accent);background:var(--btn-bg);color:var(--accent);font-size:10px;font-weight:bold;cursor:pointer;line-height:1;padding:0;display:inline-flex;align-items:center;justify-content:center;transition:background .15s,border-color .15s,color .15s;"
          onmouseover="this.style.background='var(--accent-bg)';this.style.borderColor='var(--accent-hover)';this.style.color='var(--accent-hover)';"
          onmouseout="this.style.background='var(--btn-bg)';this.style.borderColor='var(--accent)';this.style.color='var(--accent)';">i</button>
      </div>
      <div style="padding:6px 10px;display:flex;flex-direction:column;gap:4px">
        <div style="display:flex;gap:6px;align-items:center">
          <input type="text" id="set-startup-args"
            value="${(eziSettings.startupArgs||'').replace(/"/g,'&quot;')}"
            placeholder="e.g. --port 8601 --max-upload-size 4000"
            style="flex:1;min-width:0;background:var(--input-bg);color:var(--text);border:1px solid var(--border2);border-radius:4px;font-family:inherit;font-size:11px;padding:3px 6px;outline:none;"
            onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
          <button id="startup-args-clear-btn" title="Clear"
            style="flex-shrink:0;padding:3px 7px;font-family:inherit;font-size:12px;font-weight:bold;border:1px solid var(--border3);border-radius:4px;background:var(--btn-bg);color:var(--text2);cursor:pointer;transition:all .15s;line-height:1.4;"
            onmouseover="this.style.borderColor='#ff5555';this.style.color='#ff5555';"
            onmouseout="this.style.borderColor='var(--border3)';this.style.color='var(--text2)';"> &#x2715;</button>
        </div>
        <div id="startup-args-info-panel" style="display:none;margin-top:4px;background:var(--input-bg);border:1px solid var(--border2);border-radius:4px;padding:8px 10px;font-size:10px;color:var(--text2);line-height:1.7;max-height:260px;overflow-y:auto;white-space:pre-wrap;font-family:'Consolas',monospace">
<b style="color:var(--accent)">Network &amp; Server</b>
  --listen [IP]                   Listen IP (default: 127.0.0.1); bare flag = 0.0.0.0,::
  --port PORT                     Listen port (default: 8188)
  --tls-keyfile PATH              TLS key file (enables HTTPS)
  --tls-certfile PATH             TLS certificate file
  --enable-cors-header [ORIGIN]   Enable CORS (default origin: *)
  --max-upload-size MB            Max upload size in MB
  --comfy-api-base URL            ComfyUI API base URL
  --database-url URL              Database URL (e.g. sqlite:///memory)
  --enable-assets                 Enable assets system
  --enable-compress-response-body Compress HTTP responses
  --multi-user                    Per-user storage

<b style="color:var(--accent)">Paths</b>
  --temp-directory PATH           Override temp directory
  --extra-model-paths-config PATH Extra model_paths YAML file(s)
  --front-end-version SPEC        Frontend version (owner/repo@version or "latest")
  --front-end-root PATH           Local frontend root (overrides --front-end-version)

<b style="color:var(--accent)">GPU / VRAM</b>
  --cuda-device ID[,ID]           CUDA device id(s) to use
  --default-device ID             Default CUDA device
  --gpu-only                      Everything on GPU
  --highvram                      Keep models in GPU memory
  --lowvram                       Text encoders on CPU
  --novram                        When lowvram is not enough
  --cpu                           Use CPU for everything (slow)
  --reserve-vram GB               Reserve VRAM for OS/other apps
  --async-offload [N]             Async weight offloading (default 2 streams)
  --disable-async-offload         Disable async offloading
  --disable-dynamic-vram          Disable dynamic VRAM
  --enable-dynamic-vram           Force-enable dynamic VRAM
  --fast-disk                     Prefer disk-backed dynamic loading (fast NVMe)
  --disable-smart-memory          Aggressive RAM offload

<b style="color:var(--accent)">Precision</b>
  --force-fp32 / --force-fp16     Force fp32 or fp16 globally
  --fp32-unet / --fp64-unet / --bf16-unet / --fp16-unet   Diffusion model precision
  --fp8_e4m3fn-unet / --fp8_e5m2-unet / --fp8_e8m0fnu-unet  UNet fp8 variants
  --fp16-vae / --fp32-vae / --bf16-vae / --cpu-vae        VAE precision
  --fp8_e4m3fn-text-enc / --fp8_e5m2-text-enc             Text encoder fp8
  --fp16-text-enc / --fp32-text-enc / --bf16-text-enc     Text encoder precision
  --fp16-intermediates            Experimental fp16 intermediate tensors

<b style="color:var(--accent)">Attention</b>
  --use-split-cross-attention     Split cross-attention optimisation
  --use-quad-cross-attention      Sub-quadratic cross-attention
  --use-pytorch-cross-attention   PyTorch 2.0 cross-attention
  --use-sage-attention            SageAttention
  ...
  --force-channels-last           Force channels-last inference

<b style="color:var(--accent)">Caching</b>
  --cache-ram [GB GB]             RAM pressure caching (default)
  --cache-classic                 Old aggressive caching
  --cache-lru N                   LRU cache (N node results)
  --cache-none                    No caching (minimal RAM)

<b style="color:var(--accent)">Preview</b>
  --preview-method [none|auto|latent2rgb|taesd]  Sampler preview method
  --preview-size PX               Max preview size

<b style="color:var(--accent)">Manager</b>
  --enable-manager                Enable ComfyUI-Manager
  --disable-manager-ui            Disable Manager UI only
  --enable-manager-legacy-ui      Use legacy Manager UI

<b style="color:var(--accent)">Custom Nodes</b>
  --disable-all-custom-nodes      Disable all custom nodes
  --whitelist-custom-nodes FOLDER Allow specific custom node folders
  --disable-api-nodes             Disable API nodes (blocks internet)

<b style="color:var(--accent)">Backend / Performance</b>
  --cuda-malloc                   Enable cudaMallocAsync
  --disable-cuda-malloc           Disable cudaMallocAsync
  --directml [DEVICE]             Use torch-directml
  --oneapi-device-selector STR    OneAPI device selector
  --supports-fp8-compute          Treat device as fp8-capable
  --enable-triton-backend         Enable Triton backend
  --fast [OPTS]                   Untested speed optimisations
  --force-non-blocking            Non-blocking tensor ops
  --deterministic                 Deterministic (slower) algorithms
  --disable-pinned-memory         Disable pinned memory
  --mmap-torch-files              mmap for .ckpt/.pt files
  --disable-mmap                  Disable mmap for safetensors
  --default-hashing-function ALG  Hash function: md5/sha1/sha256/sha512

<b style="color:var(--accent)">Misc</b>
  --auto-launch                   Open browser on startup
  --disable-auto-launch           Don't open browser
  --disable-metadata              Don't save prompt metadata in files
  --verbose [LEVEL]               Logging level: DEBUG/INFO/WARNING/ERROR/CRITICAL
  --log-stdout                    Send output to stdout instead of stderr
  --dont-print-server             Suppress server output
  --feature-flag KEY[=VALUE]      Set a server feature flag
        </div>
      </div>
    </div>
  ...
  let _initialSecurityLevel = '';
  let _initialStartupArgs = null;
  ...
  function _checkChanges() {
    ...
    const secSel = document.getElementById('set-security-level');
    const secChanged = secSel && _initialSecurityLevel && secSel.value !== _initialSecurityLevel;

    const startupArgsEl = document.getElementById('set-startup-args');
    const startupArgsChanged = _initialStartupArgs !== null && startupArgsEl && startupArgsEl.value.trim() !== _initialStartupArgs;

    const applyBtn = Array.from(document.querySelectorAll('#modal-btns .modal-btn')).find(b => b.textContent.trim() === 'Apply');
    if (applyBtn) {
      const hasChanges = hideDepChanged || fbChanged || themeChanged || comfyChanged || feChanged || detachChanged || cpChanged || conBgChanged || secChanged || startupArgsChanged;
  ...
  function _applySettings() {
    ...
    const startupArgsEl2 = document.getElementById('set-startup-args');
    const startupArgsVal = startupArgsEl2 ? startupArgsEl2.value.trim() : (_initialStartupArgs || '');
    const startupArgsChanged2 = _initialStartupArgs !== null && startupArgsVal !== _initialStartupArgs;
    if (startupArgsChanged2) {
      eziSettings.startupArgs = startupArgsVal;
      pywebview.api.set_startup_args(startupArgsVal);
      pywebview.api.get_startup_args().then(function(fresh) {
        _initialStartupArgs = fresh || '';
        const el = document.getElementById('set-startup-args');
        if (el) el.value = _initialStartupArgs;
        _checkChanges();
      }).catch(function() { _initialStartupArgs = startupArgsVal; });
      document.getElementById('modal-overlay').classList.remove('active');
      var saNotice = document.createElement('div');
      saNotice.style.cssText = 'position:fixed;bottom:18px;left:50%;transform:translateX(-50%);background:#1c3a1c;border:1px solid #50fa7b;color:#50fa7b;padding:8px 20px;border-radius:6px;font-size:12px;font-family:inherit;z-index:999999;pointer-events:none;';
      saNotice.textContent = '⟳ Startup arguments updated — ComfyUI is restarting...';
      document.body.appendChild(saNotice);
      setTimeout(function() { if (saNotice.parentNode) saNotice.parentNode.removeChild(saNotice); }, 5000);
      return;
    }
    ...
    const secSel2 = document.getElementById('set-security-level');
    const secChanged2 = secSel2 && _initialSecurityLevel && secSel2.value !== _initialSecurityLevel;
    if (secChanged2) {
      pywebview.api.set_security_level(secSel2.value);
      document.getElementById('modal-overlay').classList.remove('active');
      var secNotice = document.createElement('div');
      secNotice.style.cssText = 'position:fixed;bottom:18px;left:50%;transform:translateX(-50%);background:#1c3a1c;border:1px solid #50fa7b;color:#50fa7b;padding:8px 20px;border-radius:6px;font-size:12px;font-family:inherit;z-index:999999;pointer-events:none;';
      secNotice.textContent = '⟳ Security level updated — ComfyUI is restarting...';
      document.body.appendChild(secNotice);
      setTimeout(function() { if (secNotice.parentNode) secNotice.parentNode.removeChild(secNotice); }, 5000);
      return;
    }
    const selComfy = document.getElementById('set-comfy-ver');
    const comfyChanged = selComfy && !selComfy.disabled && selComfy.dataset.current !== selComfy.value;
    ...
    if (hideDepChanged || fbChanged || themeChanged || comfyChanged || feChanged || detachChanged || cpChanged || conBgChanged || secChanged2 || startupArgsChanged2) {
      document.getElementById('modal-overlay').classList.remove('active');
    }
  }

  showModal('', '',
    `<div style="min-width:552px">
${tabBar}${tabGeneral}${tabAddons}${tabTorch}${tabTools}${tabAppearance}${tabAdvanced}</div>`,
    [
  ...
  function _cpRefreshFromBat() {
    pywebview.api.get_startup_args().then(function(val) {
      const el = document.getElementById('set-startup-args');
      if (el) el.value = val || '';
      _initialStartupArgs = val || '';
      _checkChanges();
    }).catch(function(){});
  ...
  function _updateModalBtns(tabName) {
    ...
    if (applyBtn) applyBtn.style.display = (tabName === 'advanced' || tabName === 'appearance') ? '' : 'none';
    if (copyBtn)  copyBtn.style.display  = (tabName === 'general')  ? '' : 'none';
    if (tabName === 'advanced' || tabName === 'appearance') _checkChanges();
  }
  ...
  setTimeout(() => {
    ...
    const secSel3 = document.getElementById('set-security-level');
    if (secSel3) {
      pywebview.api.manager_config_exists().then(function(exists) {
        if (!exists) {
          const secSection = document.getElementById('sec-level-section');
          if (secSection) secSection.style.display = 'none';
          return;
        }
        pywebview.api.get_security_level().then(function(level) {
          if (!level) level = 'weak';
          _initialSecurityLevel = level;
          secSel3.value = level;
          const loadingEl2 = document.getElementById('set-security-loading');
      ...
      secSel3.addEventListener('change', _checkChanges);
    }

    const startupArgsInput = document.getElementById('set-startup-args');
    if (startupArgsInput) {
      startupArgsInput.addEventListener('input', _checkChanges);
    }
    const startupArgsClear = document.getElementById('startup-args-clear-btn');
    if (startupArgsClear) {
      startupArgsClear.addEventListener('click', function() {
        const el = document.getElementById('set-startup-args');
        if (el) { el.value = ''; _checkChanges(); }
      });
    }
    const startupArgsInfoBtn = document.getElementById('startup-args-info-btn');
    const startupArgsInfoPanel = document.getElementById('startup-args-info-panel');
    if (startupArgsInfoBtn && startupArgsInfoPanel) {
      startupArgsInfoBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        const visible = startupArgsInfoPanel.style.display !== 'none';
        startupArgsInfoPanel.style.display = visible ? 'none' : 'block';
      });
    }

    const secInfoBtn = document.getElementById('sec-info-btn');
    const secTooltip = document.getElementById('sec-info-tooltip');
    if (secInfoBtn && secTooltip) {
      secInfoBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        const visible = secTooltip.style.display !== 'none';
        secTooltip.style.display = visible ? 'none' : 'block';
        if (!visible) {
          const rect = secInfoBtn.getBoundingClientRect();
          secTooltip.style.position = 'fixed';
          secTooltip.style.left = Math.min(rect.left, window.innerWidth - 340) + 'px';
          secTooltip.style.top = (rect.bottom + 4) + 'px';
          function _closeTooltip(ev) {
            if (!secTooltip.contains(ev.target) && ev.target !== secInfoBtn) {
              secTooltip.style.display = 'none';
              document.removeEventListener('click', _closeTooltip);
            }
          }
          setTimeout(function() { document.addEventListener('click', _closeTooltip); }, 0);
        }
      });
    }
    ...
    function _pinnedStatusIcon(status) {
      if (status === 'ok')       return '<span style="color:#50fa7b;font-size:13px" title="Version matches">&#x2714;</span>';
      if (status === 'mismatch') return '<span style="color:#ff5555;font-size:13px" title="Version mismatch">&#x2718;</span>';
      if (status === 'missing')  return '<span style="color:#ff9944;font-size:13px" title="Package not found">&#x3f;</span>';
      return '<span style="color:var(--text2);font-size:13px" title="Unknown">&#x2022;</span>';
    }

    function _renderPinnedList() {
      const container = document.getElementById('pinned-list');
      if (!container) return;
      if (!_pinnedPackages.length) {
        container.innerHTML = '<span style="font-size:11px;color:var(--text2);font-style:italic">No pinned packages.</span>';
        return;
      }
      container.innerHTML = _pinnedPackages.map(function(p, i) {
        return '<div style="display:flex;align-items:center;gap:6px;padding:3px 0;border-bottom:1px solid var(--border)">' +
          '<span id="pin-status-' + i + '" style="flex-shrink:0;width:16px;text-align:center">' +
            '<span style="color:var(--text2);font-size:10px">…</span>' +
          '</span>' +
          '<span style="flex:1;font-size:11px;color:var(--text);font-family:inherit">' + p + '</span>' +
          '<button data-pin-idx="' + i + '" class="pin-remove-btn" title="Remove"' +
            ' style="flex-shrink:0;padding:1px 7px;font-family:inherit;font-size:12px;font-weight:bold;border:1px solid var(--border3);border-radius:4px;background:var(--btn-bg);color:var(--text2);cursor:pointer;line-height:1.4;transition:border-color .15s,color .15s;"' +
            ' onmouseover="this.style.borderColor=\'#ff5555\';this.style.color=\'#ff5555\';"' +
            ' onmouseout="this.style.borderColor=\'var(--border3)\';this.style.color=\'var(--text2)\';">&#x2715;</button>' +
        '</div>';
      }).join('');
      container.querySelectorAll('.pin-remove-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var idx = parseInt(this.dataset.pinIdx);
          _pinnedPackages.splice(idx, 1);
          pywebview.api.set_pinned_packages(JSON.stringify(_pinnedPackages)).catch(function(){});
          _renderPinnedList();
          _refreshPinnedStatuses();
        });
      });
    }

    function _refreshPinnedStatuses() {
      if (!_pinnedPackages.length) return;
      pywebview.api.check_pinned_packages().then(function(json) {
        var results;
        try { results = JSON.parse(json); } catch(e) { return; }
        results.forEach(function(r, i) {
          var el = document.getElementById('pin-status-' + i);
          if (el) el.innerHTML = _pinnedStatusIcon(r.status);
        });
      }).catch(function(){});
    }

    pywebview.api.get_pinned_packages().then(function(json) {
      try { _pinnedPackages = JSON.parse(json) || []; } catch(e) { _pinnedPackages = []; }
      _renderPinnedList();
      _refreshPinnedStatuses();
    }).catch(function() { _renderPinnedList(); });

    const pinnedAddBtn = document.getElementById('pinned-add-btn');
    if (pinnedAddBtn) {
      const pinnedInp = document.getElementById('pinned-new-input');

      function _validatePinnedInput(val) {
        if (!val) return null;
        if (!/^[A-Za-z0-9_\-\.]+==[\w\.\-\+]+$/.test(val))
          return 'Use the format: package==version (e.g. numpy==1.26.4)';
        return null;
      }

      function _setPinnedError(msg) {
        var errEl = document.getElementById('pinned-input-error');
        if (!errEl) {
          errEl = document.createElement('span');
          errEl.id = 'pinned-input-error';
          errEl.style.cssText = 'font-size:10px;color:#ff5555;margin-top:1px;display:block';
          pinnedInp.parentNode.parentNode.insertBefore(errEl, pinnedInp.parentNode.nextSibling);
        }
        if (msg) {
          errEl.textContent = msg;
          errEl.style.display = 'block';
          pinnedInp.style.borderColor = '#ff5555';
        } else {
          errEl.style.display = 'none';
          pinnedInp.style.borderColor = 'var(--border2)';
        }
      }

      if (pinnedInp) pinnedInp.addEventListener('input', function() { _setPinnedError(null); });

      pinnedAddBtn.addEventListener('click', function() {
        if (!pinnedInp) return;
        const val = pinnedInp.value.trim();
        if (!val) return;
        const err = _validatePinnedInput(val);
        if (err) { _setPinnedError(err); return; }
        _setPinnedError(null);
        if (_pinnedPackages.indexOf(val) === -1) {
          _pinnedPackages.push(val);
          pywebview.api.set_pinned_packages(JSON.stringify(_pinnedPackages)).catch(function(){});
          pinnedInp.value = '';
          _renderPinnedList();
          _refreshPinnedStatuses();
  ...
  pywebview.api.get_system_info().then(function(s) {
    ...
    const activeTab = ['general','addons','appearance','advanced'].find(t => {
...
function load_ui(url) {
  ...
  frame.onload = () => {
    ...
    setTimeout(function() {
      ...
      if (eziNotice && eziNotice.style.display !== 'none') return;
    ...
    setTimeout(function() {
      ...
      var comfyNotice = document.getElementById('comfy-update-notice');
      if (comfyNotice && comfyNotice.style.display !== 'none') return;
      var wrapNotice = document.getElementById('update-notice');
      if (wrapNotice) wrapNotice.style.display = 'none';
    }, 20000);
...
Object.defineProperty(window, 'app', {
    get: function() {
        try {
            var f = document.getElementById('ui-frame');
            return f && f.contentWindow && f.contentWindow.app;
...
(function() {
    ...
    document.getElementById = function(id) {
        ...
        if (el) return el;
        try {
            var f = _orig('ui-frame');
            if (f && f.contentWindow && f.contentWindow.document) {
                return f.contentWindow.document.getElementById(id);
            }
        } catch(e) {}
        return null;
    };
            ...
            try:
                ...
                while _i2 < len(new_extra):
                    ...
                    if _tok.startswith('-'):
                        ...
                        _i2 += 1
                _managed_all = MANAGED_WITH_VAL | MANAGED_FLAG
                _seen_flags = set()
                _deduped = []
            ...
            def _port_busy(port):
                try:
                    result = subprocess.run(
                        ['powershell', '-NoProfile', '-Command',
                         f'if (Get-NetTCPConnection -LocalPort {port} -State Listen '
                         f'-ErrorAction SilentlyContinue) {{ 1 }} else {{ 0 }}'],
                        capture_output=True, timeout=5, creationflags=0x08000000
                    )
            ...
            try:
                ...
                for _entry in _pinned:
                    ...
                    if _chk.returncode != 0:
                        ...
                        for _line in _inst.stdout:
                            self._print(_line.decode('utf-8', errors='replace'))
                        _inst.wait()
